...
struct
{
	CMatrix4x4 gViewMatrix;/*MISSING LINE, not 0. This structure is used to copy data to the GPU, so look at the shader version of this structure*/;
    ...
    float      padding1;      // IMPORTANT technical point: shaders work with float4 values. If our variables don't align to
                              ...
                              // adding these unused padding variables to both HLSL and C++ structures.
...
void RenderScene()
{
    ...
    // Send per-frame data (camera matrices and lighting) over to the shaders on the GPU
    ...
    gD3DContext->Unmap(gPerFrameConstantBuffer, 0);  // This line is missing to remind you how matrices that are created here
                                                  // in C++ (in UpdateScene) are passed over to the GPU with this code snippet.
                                                  // Find the other usage of Map and Unmap in the code to see how to fix this line
...
void UpdateScene(float frameTime)
{
    ...
    gPerFrameConstants.ambientColour = {0.2f, 0.2f, 0.3f};
    gPerFrameConstants.lightColour   = {1.0f, 0.8f, 0.4f};
    gPerFrameConstants.lightPosition = {5.0f, 5.0f, -5.0f};
    gPerFrameConstants.specularPower = 64.0f;


    //// Update camera ////

    static CVector3 cameraPosition = {0, 0, -4};

    // Create a matrix to position the camera in the world (the camera's view of the world)
    gPerFrameConstants.gViewMatrix = InverseAffine(MatrixTranslation(cameraPosition));/*MISSING after fixing the first missing line in this code you should be able to see what to put here*/ 

    // Create a "projection matrix" - this determines properties of the camera like field of view and near clip distance
    gPerFrameConstants.projectionMatrix = MakeProjectionMatrix(); // Using a helper function to make this special matrix

    gPerFrameConstants.viewProjectionMatrix = gPerFrameConstants.gViewMatrix * gPerFrameConstants.projectionMatrix;

    // Simple right-left motion for camera
    if (KeyHeld(Key_Left))
    {
        cameraPosition.x -= 4.0f * frameTime;
    }
    ...
    // Send camera position over to shaders - this is needed for the specular light calculation
    gPerFrameConstants.cameraPosition = cameraPosition;
