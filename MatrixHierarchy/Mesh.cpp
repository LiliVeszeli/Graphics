...
void Mesh::RenderRecursive(std::vector<CMatrix4x4>& modelMatrices, unsigned int nodeIndex, CMatrix4x4 parentWorldMatrix /*= MatrixIdentity()*/)
{
...
//
      for (int i = 0; i < mNodes[nodeIndex].childNodes.size(); i++)
      {
           RenderRecursive(modelMatrices, mNodes[nodeIndex].childNodes[i], nodeWorldMatrix);
      }
}
