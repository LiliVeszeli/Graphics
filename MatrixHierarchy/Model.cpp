...
void Model::Control(int node, float frameTime, KeyCode turnUp, KeyCode turnDown, KeyCode turnLeft, KeyCode turnRight,
                                               KeyCode turnCW, KeyCode turnCCW, KeyCode moveForward, KeyCode moveBackward, KeyCode spinBackWheel, KeyCode spinFrontWheel, KeyCode RobotMoveLeftLegForward, KeyCode RobotMoveRightLegForward)
{
	...
	//wheel spinning
	if (KeyHeld(spinBackWheel))
	{
		mWorldMatrices[2] = MatrixRotationX(ROTATION_SPEED * frameTime) * mWorldMatrices[2];
	}
	if (KeyHeld(spinFrontWheel))
	{
		mWorldMatrices[1] = MatrixRotationX(ROTATION_SPEED * frameTime) * mWorldMatrices[1];
	}
}
...
void Model::ControlRobot(int node, float frameTime, KeyCode turnUp, KeyCode turnDown, KeyCode turnLeft, KeyCode turnRight,
	KeyCode turnCW, KeyCode turnCCW, KeyCode moveForward, KeyCode moveBackward, KeyCode RobotMoveLeftLegForward, KeyCode RobotMoveRightLegForward)
{
	auto& matrix = mWorldMatrices[node]; // Use reference to node matrix to make code below more readable
	...
	//robot moving
	if (KeyHeld(RobotMoveLeftLegForward))
	{
		mWorldMatrices[12] = MatrixRotationZ(-ROTATION_SPEED * frameTime) * mWorldMatrices[12];
		mWorldMatrices[24] = MatrixRotationZ(ROTATION_SPEED * frameTime) * mWorldMatrices[24];
		mWorldMatrices[5] = MatrixRotationZ(ROTATION_SPEED * frameTime) * mWorldMatrices[5];
		mWorldMatrices[6] = MatrixRotationZ(-ROTATION_SPEED * frameTime) * mWorldMatrices[6];

		mWorldMatrices[0].SetRow(3, mWorldMatrices[0].GetRow(3) + localZDir * MOVEMENT_SPEED * frameTime);
	}

	if (KeyHeld(RobotMoveRightLegForward))
	{
		mWorldMatrices[24] = MatrixRotationZ(-ROTATION_SPEED * frameTime) * mWorldMatrices[24];
		mWorldMatrices[12] = MatrixRotationZ(ROTATION_SPEED * frameTime) * mWorldMatrices[12];
		mWorldMatrices[5] = MatrixRotationZ(-ROTATION_SPEED * frameTime) * mWorldMatrices[5];
