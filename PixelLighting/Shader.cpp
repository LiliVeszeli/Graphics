...
ID3D11VertexShader* gLitObjectVertexShader  = nullptr;
ID3D11PixelShader*  gLitObjectPixelShader   = nullptr;
...
bool LoadShaders()
{
    ...
    gLitObjectVertexShader  = LoadVertexShader("TransformLighting_vs"); // Note how the shader files are named to show what type they are
    gLitObjectPixelShader   = LoadPixelShader ("TextureLighting_ps"  );
    gLightModelVertexShader = LoadVertexShader("TransformOnly_vs");
    gLightModelPixelShader  = LoadPixelShader ("TextureColour_ps");

    if (gLitObjectVertexShader  == nullptr || gLitObjectPixelShader   == nullptr ||
...
void ReleaseShaders()
{
    ...
    if (gLitObjectVertexShader)   gLitObjectVertexShader->Release();
    if (gLitObjectPixelShader)    gLitObjectPixelShader->Release();
}
