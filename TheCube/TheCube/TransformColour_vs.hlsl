...
// This structure is a constant buffers - a way of passing data over from C++ to the GPU every frame.
...
// Note we don't need the name of the constant buffer to access the variables inside, so we can just write gWorldMatrix for example
